...
void LoggableTest::doChangeName(const std::string& newName) {
  changeName(newName);
}
...
TEST_CASE("Loggable logs correctly", "[logging][loggable]") {
  ...
  SECTION("Info logging works correctly") {
    ...
    auto combinedString =
        dateRegexString + infoRegexString + idRegexString + messageRegexString;
  ...
  SECTION("Warning logging works correctly") {
    ...
    auto combinedString =
        dateRegexString + warnRegexString + idRegexString + messageRegexString;
  ...
  SECTION("Error logging works correctly") {
    ...
    auto combinedString =
        dateRegexString + errorRegexString + idRegexString + messageRegexString;
  ...
  SECTION("Debug logging works correctly") {
    ...
    auto combinedString =
        dateRegexString + debugRegexString + idRegexString + messageRegexString;
  ...
  SECTION("Change Name works correctly") {
    ...
    auto newMessageRegexString =
        std::string(R"(\[testName2\](\s)test1(\s)test2\n)");
    auto logged = *backend->getLog();
    auto infoRegexString = std::string(R"(\[Info\])");
    auto combinedString = dateRegexString + infoRegexString + idRegexString +
                          newMessageRegexString;
    auto infoRegex = std::regex(combinedString);
    REQUIRE(std::regex_match(logged, infoRegex));
  }
  SECTION("Fatal logging works correctly") {
    ...
    auto combinedString =
        dateRegexString + fatalRegexString + idRegexString + messageRegexString;
