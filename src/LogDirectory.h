...
// Directory class that counts files in it and enforces a limit on filecount of
// files with correct extension, deleting the oldest files. If Limit is Zero or
// negative don't enforce
class LogDirectory {
 public:
  LogDirectory(std::filesystem::path& path, std::string extension,
               int fileLimit);
